...
class DocumentProcessor:
    ...
    def _prepare_image(self):
        ...
        print(f"    - Image quality: {quality_info['quality_preset']} (denoise={quality_info['denoising_h']})")
    ...
    def _extract_text_blocks(self):
        ...
        # --- LOAD DESCRIPTION GENERATOR FOR SPELL-CHECK ---
        ...
        for i, block_data in enumerate(detected_elements):
            if 'layout_type' in block_data:
                ...
                if element_type == 'Table':
                    ...
                    textblock_counter += 1
                    if config.USE_OCR_ROUTING and hasattr(ocr, 'ocr_routed'):
                        text, conf = ocr.ocr_routed(self.bgr_image, self.binary_image, box, self.detected_lang_tess)
                    else:
                        text, conf = ocr.ocr_smart(self.bgr_image, self.binary_image, box, self.detected_lang_tess)
                    
                    # === SAFE LLM CORRECTION INTEGRATION ===
                    # Only verify if desc_gen is available and text is somewhat low confidence but not garbage
                ...
                if config.USE_OCR_ROUTING and hasattr(ocr, 'ocr_routed'):
                    ...
                    # Use existing text from Tesseract but assume unknown confidence
                    conf = 0.0 
                
                # === SAFE LLM CORRECTION INTEGRATION ===
                if desc_gen and len(text) > 10:
                     # Heuristic: if text has mixed case errors or double spaces, try fixing
                     if "  " in text or (not text[0].isupper() and len(text)>3):
                        text = desc_gen.correct_ocr_mistakes(text)
                # =======================================
    ...
    def _extract_figures(self):
        ...
        if not figures_found:
            ...
            for i, box in enumerate(figure_boxes):
                ...
                if non_text_content_density < 0.05: continue
    ...
    def _extract_fields(self):
        """Finds key-value pairs using hybrid rule-based + LLM approach."""
        ...
        if description_generator and description_generator.pipe:
            self.metadata['fields'] = field_extraction.find_fields_hybrid(
                text_blocks, description_generator
            )
        else:
